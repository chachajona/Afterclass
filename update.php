...
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['fdid'] && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $id = @$_GET['fdid'];
        $result = mysqli_query($con, "DELETE FROM feedback WHERE id='$id' ") or die('Error');
        header("location:adminDashboard.php?q=3");
    }
}

if (isset($_SESSION['accessToken'])) {
    if (@$_GET['deidquiz'] && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $eid = @$_GET['deidquiz'];
...
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['eeidquiz'] && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $eid = @$_GET['eeidquiz'];
...
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['dusername'] && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        ...
        header("location:adminDashboard.php?q=1");
    }
}

//Remove Quiz
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['q'] == 'rmquiz' && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $eid = @$_GET['eid'];
        ...
        header("location:adminDashboard.php?q=5");
    }
}

//Add Quiz
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $name    = $_POST['name'];
        ...
        header("location:adminDashboard.php?q=4&step=2&eid=$id&n=$total");
    }
}

if (isset($_SESSION['accessToken'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $n   = @$_GET['n'];
        $eid = @$_GET['eid'];
        $ch  = @$_GET['ch'];
        for ($i = 1; $i <= $n; $i++) {
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_POST['ans']) && (!isset($_GET['delanswer']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header("location:studentDashboard.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total") or die('Error152');
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && (!isset($_POST['ans'])) && (isset($_GET['delanswer'])) && $_GET['delanswer'] == "delanswer") {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:studentDashboard.php?q=quiz&step=2&eid=' . $_GET['eid'] . '&n=' . $_GET['n'] . '&t=' . $total);
...
//Remove Course
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['q'] == 'rmcourse' && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $result = mysqli_query($con, "DELETE FROM courses WHERE cid={$_GET['cid']}") or die('Error');
        header("location:adminDashboard.php?q=6");
    }
}

//Add Course
if (isset($_SESSION['accessToken'])) {
    if (@$_GET['q'] == 'addcourse' && $_SESSION['accessToken'] == '751cb3f4aa17c36186f4856c8982bf27') {
        $course_name    = $_POST['course_name'];
        $course_desc   = $_POST['course_desc'];
        $course_author = $_POST['course_author'];
        $course_duration   = $_POST['course_duration'];
        $course_image    = $_FILES['course_img']['name'];
        $course_image_temp    = $_FILES['course_img']['tmp_ame'];
        $img_folder = './image/courseImages/'.$course_image;
        move_uploaded_file($course_image_temp, $img_folder);

        $id      = uniqid();
        $q      = mysqli_query($con, "INSERT INTO courses VALUES(NULL,'$id','$course_name','$course_desc','$course_author','$img_folder','$course_duration')");
        header("location:adminDashboard.php?q=6&step=2&cid=$id");
    }
}
?>
